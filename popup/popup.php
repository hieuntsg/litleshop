    ...
    <script>
        ...
        $(window).load(function () {
            ...
            $(".c-pdi").click(function(){
                //messageDiaLog('Thông báo','Thêm giỏ hàng thành công','');
                //alert(document.getElementsByName('productid').val);
                //alert(document.getElementsByName('command').val);
                document.form1.submit();

            })

            $(".xemgia").click(function(){
                messageDiaLog('Thông báo','Bạn vui lòng đăng nhập để xem giá.<br><br>Nếu chưa có tài khoản <a href="../sigin.html" target="_top">Đăng kí tại đây</a>','');
            })
            ...
            /*}*/
             ...
             if ($.isMobile) {
                ...
                //$('.zoomWrapper').css('display','none');
                 $("#listImageSlide").css('display','none');
            }else{
                var wZ = $("#demo-container").width();
                var hZ = $("#demo-container").height();
...
.prev-product
{
    ...
    z-index: 10;
        ...
        ?>        
                <a class="item "  data-image="<?=_upload_sanpham.$vc['photo']?>" data-zoom-image="<?=_upload_sanpham.$vc['photo']?>" style="display: block; margin: 0px; height: 264px; width: 100px;padding-top: 5px;"><img src="<?=_upload_sanpham.$vc['photo']?>"></a>
        <?php } } ?>        
...
?>
<form id="form1" name="form1" target="_top" action="../index.php">
            <div  class="ghsp"  >
        
                <div class="product-detail-item">
                <div class="n-pdi"><?=$result_list['ten']?></div>
                <div class="d-pdi" style="line-height: 20px;"><b>Mô tả : </b> <?=$result_detail_pro['mota']?></div>
                ...
                <tr>
                    ...
                    <select class="s-pdi" name="size">
                        <option value="Free" >Free</option>
                    </select>
                    <?php }else{?>
                    <select class="s-pdi" name="size">
                        <?php foreach ($result_size as $kz => $vz) {?>
                            <option value="<?=$vz['ten']?>" ><?=$vz['ten']?></option>
                        <?php } ?>       
                    </select>
                    <?php } ?>
                    </td>
                ...
                <tr>
                    ...
                    <td>Số lượng :
                    <select class="q-pdi" id="quantity" name="quantity">
                    <?php for ($i=1; $i <= $result_detail_pro['soluong']; $i++) { ?>
            ...
            <?php if($result_detail_pro['soluong'] == 0 ||$result_detail_pro['soluong'] == ''){?>    
                <a class="c-d-pdi"  style="display: block;">Hết hàng</a>
             <?php }else{ ?>
                <?php  if(empty($_SESSION['mem_login']['mem_mail'])){ ?>
                <?php } else{ ?>
                 <a  class="c-pdi"  style="display: block;">Thêm giỏ</a>
                <? } } ?>     
                </div>
            ...
            <div runnat="Div"  class="Div guide-product">
