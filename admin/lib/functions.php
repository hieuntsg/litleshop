...
function format_money($gia)
{
	if($gia>0){
		$str= number_format($gia,0,",",",")." VNĐ";
		}else{$str ="Liên hệ" ;}
		return $str;
}
...
function paging_home($r, $url='', $curPg=1, $mxR=5, $mxP=5, $class_paging='')
	{
		...
		if($totalRows>$mxR)
		{
			...
			#{
				...
				$paging .=" <a href='".$url."&p=".($totalPages)."' class=\"{$class_paging}\" >&raquo;</a> <div class='clear'></div>"; //ve cuoi
			#}
		}
		$r3['curPage']=$curPg;
		$r3['source']=$r2;
...
function check_yahoo($nick_yahoo){
	...
	if($read==false || strstr($read,"00"))
		$img = '<img  src="images/o_yahoo.png" border="0" />';
	else
		$img = '<img  src="images/f_yahoo.png"  border="0" />';
