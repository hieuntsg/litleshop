...
function load_level(){
	...
	if($level<$type){
		...
		$rs=$d->result_array();
		if($d->num_rows()>0){
		$str='<b>Chọn danh mục cấp '.$level.'</b>';
		$str.='<select name="id_parent[]" class="form-control input level'.$level.'" data-level="'.$level.'"  onchange="load_level($(this))"><option> Chọn danh mục cấp '.$level.' </option>';
		foreach($rs as $v){
