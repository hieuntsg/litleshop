...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM #_video1 where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($spdc1==0)
	{
	$sqlUPDATE_ORDER = "UPDATE #_video1 SET tinnoibat ='".$tinnoibat."' WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE #_video1 SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	$sql_sp = "SELECT id,hienthi FROM #_video1 where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE #_video1 SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE #_video1 SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");

	}	
	}
	
	$sql = "select * from #_video1 ";
	...
	if($_REQUEST['id_cat']!='')$url="index.php?com=video&act=man&id_cat=".$_REQUEST['id_cat'];
	else $url="index.php?com=video&act=man";
	$maxR=25;
	$maxP=4;
...
function get_item(){
	...
	$sql = "select * from #_video1 where id='".$id."'";
	$d->query($sql);
...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'pdf|rar|zip|doc|docx|xlsx|xls',_upload_tintuc,$file_name)){
			$data['photo'] = $photo;
			$d->setTable('video1');
			$d->setWhere('id', $id);
			$d->select();
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		if($d->num_rows()>0){
			...
			$sql = "delete from #_video1 where id='".$id."'";
			$d->query($sql);
		}
		...
		} redirect("index.php?com=video&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=news&act=man");
}
//===========================================================
function get_cats(){
	global $d, $items, $paging;
