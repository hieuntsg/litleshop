...
if(isset($_GET['id'])){
	...
	#các tin cu hon
	$sql_khac = "select ten,tenkhongdau,ngaytao,id from #_news where hienthi=1 and id_item='4' and id <>'".$id."' order by stt,ngaytao desc limit 0,5";
	$d->query($sql_khac);
	...
	$title_bar='Tin tức - ';		
	$title_tcat='Tin tức';	
	$sql_tintuc = "select ten,tenkhongdau,mota,photo,id from #_news where hienthi=1 and id_item='4' order by stt,ngaytao desc";
	$d->query($sql_tintuc);
	$tintuc = $d->result_array();
	$curPage = isset($_GET['p']) ? $_GET['p'] : 1;
	$url=getCurrentPageURL();
